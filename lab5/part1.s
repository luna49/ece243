...
_start:
    movia r8, HEX_BASE1
	movia r9, HEX_BASE2
	movia r10, 17				# Counter for the 16 HEX displays (0 to F)
	movia r11, 0				# Count up
loop:
    mov r4, r11
	movi    r5, 0            	# Display on HEX display 0 (right most one)
    call    HEX_DISP          	# Display the digit
    call	wait_delay         	# Delay
    
    subi    r10, r10, 1			# Increment the digit
	addi	r11, r11, 1
    
    bne     r10, r0, loop	# Check if all digits (0-F) have been displayed
    
    # Blanking the display
    movi    r4, 0b00010000     # Set bit 4 = 1 to blank the display

    # End of program
	movi r10, 17	# Reset count to 0 to loop
	movi r11, 0
    br loop

# Function to introduce a delay for visibility
wait_delay:
    movia    r7, 500000        # Adjust this value for desired delay
delay_loop:
    subi    r7, r7, 1
